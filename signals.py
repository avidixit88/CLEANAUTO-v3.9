...
def compute_scalp_signal(
    ...
    *,
    ...
    def _select_vwap_event_series(close_s: pd.Series, primary: pd.Series, secondary: pd.Series | None, side: str) -> tuple[pd.Series, str, bool, bool]:
        """Pick the VWAP line that actually shows the reclaim/reject behavior.

        Prefer the configured VWAP, but if the alternate VWAP clearly shows the event while
        the primary one does not, allow the alternate to drive the event. This keeps existing
        workflow/payloads intact while widening detection to cumulative-VWAP-led moves.
        """
        def _event(s: pd.Series) -> tuple[bool, bool]:
            if s is None or len(s) < 6 or len(close_s) < 6:
                return False, False
            try:
                was_below = bool((close_s.shift(3) < s.shift(3)).iloc[-1] or (close_s.shift(5) < s.shift(5)).iloc[-1])
                reclaim = bool((close_s.iloc[-1] > s.iloc[-1]) and (close_s.shift(1).iloc[-1] <= s.shift(1).iloc[-1]))
                was_above = bool((close_s.shift(3) > s.shift(3)).iloc[-1] or (close_s.shift(5) > s.shift(5)).iloc[-1])
                reject = bool((close_s.iloc[-1] < s.iloc[-1]) and (close_s.shift(1).iloc[-1] >= s.shift(1).iloc[-1]))
            except Exception:
                return False, False
            return (was_below and reclaim), (was_above and reject)

        p_long, p_short = _event(primary)
        if side == "LONG":
            if p_long:
                return primary, str(vwap_logic), True, False
            if secondary is not None:
                s_long, _ = _event(secondary)
                if s_long:
                    return secondary, "cumulative" if str(vwap_logic).lower() == "session" else "session", True, True
            return primary, str(vwap_logic), False, False
        else:
            if p_short:
                return primary, str(vwap_logic), True, False
            if secondary is not None:
                _, s_short = _event(secondary)
                if s_short:
                    return secondary, "cumulative" if str(vwap_logic).lower() == "session" else "session", True, True
            return primary, str(vwap_logic), False, False
    ...
    # VWAP event (prefer configured VWAP, but allow the alternate VWAP if that is the one
    # the market is actually reclaiming/rejecting).
    alt_vwap = df["vwap_cum"] if str(vwap_logic).lower() == "session" else df["vwap_sess"]
    long_vwap_series, long_vwap_label, reclaim_vwap, reclaim_from_alt = _select_vwap_event_series(close, vwap_use, alt_vwap, "LONG")
    short_vwap_series, short_vwap_label, reject_vwap, reject_from_alt = _select_vwap_event_series(close, vwap_use, alt_vwap, "SHORT")
    was_below_vwap = bool(reclaim_vwap)
    was_above_vwap = bool(reject_vwap)
    extras["vwap_event_long_label"] = long_vwap_label
    extras["vwap_event_short_label"] = short_vwap_label
    extras["vwap_event_long_from_alt"] = bool(reclaim_from_alt)
    extras["vwap_event_short_from_alt"] = bool(reject_from_alt)
    ...
    if was_below_vwap and reclaim_vwap:
        _add("LONG", "vwap_event", 35, f"VWAP reclaim ({long_vwap_label})")
    ...
    if was_above_vwap and reject_vwap:
        _add("SHORT", "vwap_event", 35, f"VWAP rejection ({short_vwap_label})")
...
def compute_ride_signal(
    ...
    **_ignored: object,
    ...
    alt_vwap_series = vwap_cum if str(vwap_logic).lower() == "session" else vwap_sess
    ref_vwap = float(ref_vwap_series.iloc[-1]) if len(ref_vwap_series) else None
    alt_vwap = float(alt_vwap_series.iloc[-1]) if len(alt_vwap_series) else None

    def _ride_vwap_legitimacy(primary_series: pd.Series, secondary_series: pd.Series | None):
        """Evaluate reclaim/reject on the configured VWAP, but allow the alternate VWAP
        to qualify continuation/reclaim when that is clearly the line the market respected.
        Returns long/short scores, booleans, selected anchor values, and human-readable labels.
        """
        def _eval(s: pd.Series):
            if s is None or len(s) < 6:
                return {"reclaim": False, "reject": False, "score_up": 0.0, "score_dn": 0.0}
            sv = float(s.iloc[-1])
            reclaim_cross = bool(sv is not None and prev_close <= sv and last_price > sv and disp_ok)
            reject_cross = bool(sv is not None and prev_close >= sv and last_price < sv and disp_ok)
            reclaim_hold = bool(len(close_s) >= 2 and (close_s.tail(2) > sv - 0.10 * buffer).all())
            reject_hold = bool(len(close_s) >= 2 and (close_s.tail(2) < sv + 0.10 * buffer).all())
            swept_low = bool(low_s.iloc[-1] < (prev_low_min - 0.05 * atr_last) and last_price > sv and close_q_long >= 0.55)
            swept_high = bool(high_s.iloc[-1] > (prev_high_max + 0.05 * atr_last) and last_price < sv and close_q_short >= 0.55)
            reclaim = bool(reclaim_cross and (reclaim_hold or swept_low))
            reject = bool(reject_cross and (reject_hold or swept_high))
            score_up = (0.45 if reclaim_cross else 0.0) + (0.30 if reclaim_hold else 0.0) + (0.25 if swept_low else 0.0)
            score_dn = (0.45 if reject_cross else 0.0) + (0.30 if reject_hold else 0.0) + (0.25 if swept_high else 0.0)
            return {"reclaim": reclaim, "reject": reject, "score_up": score_up, "score_dn": score_dn}

        p = _eval(primary_series)
        use_label_long = str(vwap_logic)
        use_label_short = str(vwap_logic)
        use_series_long = primary_series
        use_series_short = primary_series
        if secondary_series is not None:
            s = _eval(secondary_series)
            alt_label = "cumulative" if str(vwap_logic).lower() == "session" else "session"
            if (not p["reclaim"]) and s["reclaim"] and s["score_up"] >= p["score_up"] + 0.10:
                p["reclaim"] = True; p["score_up"] = max(p["score_up"], s["score_up"])
                use_label_long = alt_label; use_series_long = secondary_series
            if (not p["reject"]) and s["reject"] and s["score_dn"] >= p["score_dn"] + 0.10:
                p["reject"] = True; p["score_dn"] = max(p["score_dn"], s["score_dn"])
                use_label_short = alt_label; use_series_short = secondary_series
        return p, use_series_long, use_series_short, use_label_long, use_label_short
    ...
    # VWAP reclaim/reject legitimacy: prefer the configured VWAP, but allow the alternate
    # VWAP to qualify when that is the line the market actually respected.
    vwap_eval, selected_vwap_long_series, selected_vwap_short_series, selected_vwap_long_label, selected_vwap_short_label = _ride_vwap_legitimacy(ref_vwap_series, alt_vwap_series)
    vwap_reclaim = bool(vwap_eval["reclaim"])
    vwap_reclaim_cross = bool(vwap_eval["reclaim_cross"])
    vwap_reclaim_hold = bool(vwap_eval["reclaim_hold"])
    vwap_reject = bool(vwap_eval["reject"])
    vwap_reject_cross = bool(vwap_eval["reject_cross"])
    vwap_reject_hold = bool(vwap_eval["reject_hold"])
    vwap_score_up = float(vwap_eval["score_up"])
    vwap_score_dn = float(vwap_eval["score_dn"])
    selected_vwap_long = float(selected_vwap_long_series.iloc[-1]) if selected_vwap_long_series is not None and len(selected_vwap_long_series) else ref_vwap
    selected_vwap_short = float(selected_vwap_short_series.iloc[-1]) if selected_vwap_short_series is not None and len(selected_vwap_short_series) else ref_vwap
    ...
    # Secondary continuation lane folded into existing RIDE workflow:
    # If a prior impulse already happened and price has reclaimed/held VWAP with supportive ADX/DI,
    # let that strengthen the existing VWAP lane rather than inventing a new payload/workflow mode.
    cont_lookback = int(min(18, max(6, len(df))))
    recent_close_tail = close_s.tail(cont_lookback)
    recent_low_tail = low_s.tail(cont_lookback)
    recent_high_tail = high_s.tail(cont_lookback)
    adx_tail = pd.to_numeric(adx_ff.tail(int(min(4, len(adx_ff)))), errors="coerce").dropna() if len(adx_ff) else pd.Series(dtype=float)
    adx_rising_up = bool(len(adx_tail) >= 3 and float(adx_tail.iloc[-1]) >= float(adx_tail.iloc[-2]) - 0.20 and float(adx_tail.iloc[-2]) >= float(adx_tail.iloc[-3]) - 0.25)
    adx_rising_dn = bool(len(adx_tail) >= 3 and float(adx_tail.iloc[-1]) >= float(adx_tail.iloc[-2]) - 0.20 and float(adx_tail.iloc[-2]) >= float(adx_tail.iloc[-3]) - 0.25)
    prior_impulse_up_atr = float(max(0.0, float(last_price - float(recent_close_tail.min()))) / max(1e-9, float(atr_last)))
    prior_impulse_dn_atr = float(max(0.0, float(float(recent_close_tail.max()) - last_price)) / max(1e-9, float(atr_last)))
    micro_hl_up = bool(len(recent_low_tail) >= 4 and float(recent_low_tail.iloc[-1]) >= float(recent_low_tail.iloc[-2]) - 0.10 * float(atr_last) and float(recent_low_tail.iloc[-2]) >= float(recent_low_tail.iloc[-3]) - 0.14 * float(atr_last))
    micro_lh_dn = bool(len(recent_high_tail) >= 4 and float(recent_high_tail.iloc[-1]) <= float(recent_high_tail.iloc[-2]) + 0.10 * float(atr_last) and float(recent_high_tail.iloc[-2]) <= float(recent_high_tail.iloc[-3]) + 0.14 * float(atr_last))
    vwap_hold_up = bool(selected_vwap_long is not None and len(close_s) >= 3 and (close_s.tail(3) > float(selected_vwap_long) - 0.08 * float(atr_last)).all() and float(low_s.tail(3).min()) >= float(selected_vwap_long) - 0.22 * float(atr_last))
    vwap_hold_dn = bool(selected_vwap_short is not None and len(close_s) >= 3 and (close_s.tail(3) < float(selected_vwap_short) + 0.08 * float(atr_last)).all() and float(high_s.tail(3).max()) <= float(selected_vwap_short) + 0.22 * float(atr_last))
    cont_reclaim_up = bool(selected_vwap_long is not None and last_price > float(selected_vwap_long) and vwap_hold_up and prior_impulse_up_atr >= 1.55 and np.isfinite(di_p) and np.isfinite(di_m) and di_p >= di_m + 4.0 and np.isfinite(adx_last) and adx_last >= 18.0 and adx_rising_up and micro_hl_up)
    cont_reclaim_dn = bool(selected_vwap_short is not None and last_price < float(selected_vwap_short) and vwap_hold_dn and prior_impulse_dn_atr >= 1.55 and np.isfinite(di_p) and np.isfinite(di_m) and di_m >= di_p + 4.0 and np.isfinite(adx_last) and adx_last >= 18.0 and adx_rising_dn and micro_lh_dn)
    continuation_score_up = float(np.clip((0.34 if vwap_hold_up else 0.0) + (0.28 if prior_impulse_up_atr >= 1.55 else 0.0) + (0.18 if micro_hl_up else 0.0) + (0.20 if (np.isfinite(di_p) and np.isfinite(di_m) and di_p >= di_m + 4.0 and np.isfinite(adx_last) and adx_last >= 18.0 and adx_rising_up) else 0.0), 0.0, 1.0))
    continuation_score_dn = float(np.clip((0.34 if vwap_hold_dn else 0.0) + (0.28 if prior_impulse_dn_atr >= 1.55 else 0.0) + (0.18 if micro_lh_dn else 0.0) + (0.20 if (np.isfinite(di_p) and np.isfinite(di_m) and di_m >= di_p + 4.0 and np.isfinite(adx_last) and adx_last >= 18.0 and adx_rising_dn) else 0.0), 0.0, 1.0))

    impulse_scores_long = {"ORB": float(np.clip(orb_score_up, 0.0, 1.0)), "PIVOT": float(np.clip(pivot_score_up, 0.0, 1.0)), "VWAP": float(np.clip(max(vwap_score_up, continuation_score_up), 0.0, 1.0))}
    impulse_scores_short = {"ORB": float(np.clip(orb_score_dn, 0.0, 1.0)), "PIVOT": float(np.clip(pivot_score_dn, 0.0, 1.0)), "VWAP": float(np.clip(max(vwap_score_dn, continuation_score_dn), 0.0, 1.0))}
    continuation_reclaim_long = bool(cont_reclaim_up and continuation_score_up >= 0.62)
    continuation_reclaim_short = bool(cont_reclaim_dn and continuation_score_dn >= 0.62)
    ...
    long_legit_trigger = bool((orb_score_up > 0.45) or (pivot_score_up > 0.45) or vwap_reclaim or continuation_reclaim_long)
    short_legit_trigger = bool((orb_score_dn > 0.45) or (pivot_score_dn > 0.45) or vwap_reject or continuation_reclaim_short)
    ...
    continuation_reclaim_mode = bool((direction == "LONG" and continuation_reclaim_long) or (direction == "SHORT" and continuation_reclaim_short))
    ...
    def _build_ride_entry_geometry(conviction_score: float) -> dict[str, object]:
        ...
        pullback_anchor_local = float(accept_line)
        if continuation_reclaim_mode:
            # Continuation-style reclaim setups should anchor pullback geometry to the
            # reclaimed VWAP support/resistance zone (session OR cumulative), not the upper impulse leg.
            _cont_vwap = selected_vwap_long if direction == "LONG" else selected_vwap_short
            if _cont_vwap is not None:
                pullback_anchor_local = float(_cont_vwap)
        ...
        if direction == "LONG":
            ...
            pb1_local = float(pullback_anchor_local) + pb_inner_local
            pb2_local = float(pullback_anchor_local) - pb_outer_local
            pullback_entry_local = float(np.clip(float(pullback_anchor_local), pb2_local, pb1_local))
            ...
            pb1_local = float(pullback_anchor_local) - pb_inner_local
            pb2_local = float(pullback_anchor_local) + pb_outer_local
            pullback_entry_local = float(np.clip(float(pullback_anchor_local), pb1_local, pb2_local))
            stop_mult_local = 0.55 if stage == "PRE" else 0.80
            stop_local = float(pullback_entry_local + stop_mult_local * atr_last)
        ...
        if continuation_reclaim_mode and not actionable_local:
            actionable_local = bool((dist_pb_band_local <= 1.35 * prox_dist_local) and not stale_breakout_local)

        breakout_ref_local = None
        if direction == "LONG":
            if impulse_type_hint == "ORB" and orb_high is not None:
                breakout_ref_local = float(orb_high)
            elif impulse_type_hint == "VWAP":
                _br_vwap = selected_vwap_long if direction == "LONG" else selected_vwap_short
                if _br_vwap is not None:
                    breakout_ref_local = float(_br_vwap)
        ...
        if actionable_local:
            ...
            if continuation_reclaim_mode:
                if near_pullback_local:
                    choose_pullback = True
                elif near_break_local:
                    # If continuation reclaim is active but price is no longer near the
                    # reclaimed support zone, do not force a broken pseudo-pullback.
                    # Let the normal breakout lane proceed when the break trigger is the
                    # only nearby actionable geometry.
                    choose_pullback = bool(float(dist_pb_band_local) <= 0.95 * prox_dist_local)
                else:
                    choose_pullback = bool(float(dist_pb_band_local) <= 1.35 * prox_dist_local)
        ...
        return {
            ...
            "pullback_anchor": float(pullback_anchor_local),
    ...
    why.append(f"Impulse: {((selected_vwap_long_label if direction == 'LONG' else selected_vwap_short_label) if impulse_type_hint == 'VWAP' else impulse_type_hint)} L={impulse_legitimacy:.2f}")
    if continuation_reclaim_mode:
        why.append("VWAP reclaim continuation")
    try:
    ...
    return SignalResult(
        ...
        extras={
            ...
            "continuation_reclaim_mode": bool(continuation_reclaim_mode),
            "selected_vwap_long_label": selected_vwap_long_label,
            "selected_vwap_short_label": selected_vwap_short_label,
            "continuation_score": float(continuation_score_up if direction == "LONG" else continuation_score_dn),
            "prior_impulse_atr": float(prior_impulse_up_atr if direction == "LONG" else prior_impulse_dn_atr),
        },
    )
